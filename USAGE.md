...
segmentations (e.g. produced by **MemBrain-seg**) with the **Cryo3D Editor**
plugin for [napari](https://napari.org).
...
*Loading data — open **File ▸ Open File(s)…** in napari, or simply drag the file
into the window.*
...
*Pick your tomogram (`.mrc`/`.rec`) and/or segmentation; <kbd>Shift</kbd>- or
<kbd>Ctrl</kbd>-click to open several files in one go.*
...
*Each opened file becomes a layer in the list (bottom-left); the canvas shows a
single Z-slice — scroll to move through the depth of the volume.*

![Right-click layer menu](images/tut/img4.png)
*If a segmentation opens as an **Image**, right-click the layer and choose
**Convert to Labels** so the editing tools can act on it.*

When displayed as Labels, each object gets its own colour:

![Segmentation shown as coloured labels](images/tut/img5.png)
*As a **Labels** layer, every object (label ID) is drawn in its own colour.*
...
*Open the plugin from **Plugins ▸ Cryo3D Editor**.*
...
*The Cryo3D Editor panel docks on the right with five tabs; use the **layer
dropdown** at the top of each tab to choose which layer it acts on.*
...
*Scroll through the Z-slices to inspect the whole volume before editing — it
helps you understand your data's structure first.*
...
| **Erase** | 2D paint-mask eraser for Labels layers | Manually deleting wrong regions |
...
*Dust tab — the raw segmentation before cleaning, still full of small specks.*

![Preview after Hide Dust](images/tut/wf_dust_after.png)
*Dust tab — preview after **Hide Dust**: isolated specks are removed while the
real structures are kept.*

---

## 6. Erase — paint out regions
...
*Erase tab — paint a red mask over an unwanted region, then **Apply Erase** to
delete those voxels. You can also erase directly with napari's own brush tools.*
...
*Colors tab — the segmentation with each membrane/label shown in a distinct
colour, making structures easy to tell apart.*

![Selecting and changing label colours](images/tut/wf_colors.png)
*Colors tab — select a label in the list and pick a new colour; the canvas
updates live.*
...
*Render tab — **Save Snapshot…** exports exactly what is on the canvas as a
TIFF/PNG image (tick **Canvas only** to exclude the napari UI).*

![Saving the cleaned segmentation as an MRC file](images/tut/wf_save_mrc.png)
*Render tab — **Save as MRC…** writes the cleaned segmentation to a new `.mrc`
file, preserving the voxel size in the header.*
...
*Movie tab — keyframe and spin-movie controls, with the captured keyframes
listed and the preview/export buttons at the bottom.*
...
*Preview of a keyframe animation — the camera interpolates smoothly between the
captured viewpoints.*

**Full walkthrough:** [watch the complete Movie-tab demo](https://github.com/lifeviewspace/cryo3d-editor/raw/main/images/tut/movie_tab_demo.mp4)
